...
//	HWND hWindow;
	int x = 100;
	int tamCombo = 60, espCombo = 30,i;
	NR *c2;
	NP *c1;
	int idCombo;
	int posCombo, posx= 0,posy= 0;
	...
	switch (message)
	{
		...
		//	hWindow = CreateWindow( "VENTANA", 0, CBS_SIMPLE |WS_CHILD |WS_VISIBLE|WS_VSCROLL, 10,10,posx,posy,hDlg,(HMENU)IDC_NUEVO,hInst,NULL);
			c1 = lista;
		...
		case WM_COMMAND:	
			...
			switch(LOWORD(wParam))
			{
				case IDC_ACCION:
					...
					MoveWindow(hDlg, rtMarco.left, rtMarco.top,(tamCombo*numCombos)+(espCombo*numCombos) + espCombo +  espCombo/4,rtDlg.bottom, TRUE);
